...
fn registry(context: AppContext) -> Registry {
    Registry::builder()
        .register_typed_with(
            JOB_SEND_RECEIPT,
            context.clone(),
            |_job, payload: SendReceipt, context| async move {
                send_receipt(&context, payload.order_id).await
            },
        )
        .register_with(JOB_SYNC_PRICES, context, |_job, context| async move {
            sync_prices(&context).await
        })
        .build()
}
...
Every cron schedule runs in production and staging by default. Set the current environment in `WorkerConfig`; a worker with cron schedules refuses to start until it has one, so a schedule is never skipped because the environment was left unset. Use `CronJob::environments` to select a different set. Development, local, and test workers run a schedule only when the schedule includes their environment.

```rust,no_run
use std::time::Duration;

use pgspawn::{CronJob, Environment, WorkerConfig};

let payouts = CronJob::interval(
    "publisher-payouts",
    "publisher_payouts",
...
The worker does not register or enqueue `payouts` in the local environment. Use `.environments(Environment::ALL)` for a schedule that must run in every environment. The worker validates every schedule against the registry before it applies the environment, so a schedule with a missing handler fails at startup in every environment.

The environment restriction controls cron schedules only. It does not block jobs that the application enqueues directly, and it does not remove jobs already in the queue: an occurrence enqueued earlier by a worker in another environment still runs wherever its handler is registered. A cron row that an earlier worker created stays in the table; a worker whose environment excludes that schedule neither updates nor enqueues it.
...
By default, adding a new cron definition records its current schedule without enqueueing an old occurrence. Call `CronJob::catch_up_on_start` to enqueue the latest due occurrence when the definition is first observed. Use that for long intervals (hourly or daily) that previously ran once at process start; short intervals wait at most one slot after deploy, which is usually enough.

Every cron job name must have a registered handler, and cron identifiers must be unique within a worker.

## Periodic maintenance jobs

A typical control-plane worker is a set of log-and-continue cycles: the handler absorbs its own failures and the next run comes from the schedule, not from pgspawn retry. Register those with `register_infallible` or `register_infallible_with` so the job is always recorded as succeeded:

```rust,no_run
use std::time::Duration;

use pgspawn::{Registry, WorkerConfig};

#[derive(Clone)]
struct AppContext {/* db, config, ... */}

fn registry(context: AppContext) -> Registry {
    Registry::builder()
        .register_infallible_with("prune-sessions", context, |_job, context| async move {
            if let Err(error) = prune_sessions(&context).await {
...
# async fn prune_sessions(_context: &AppContext) -> Result<(), std::io::Error> { Ok(()) }

fn worker_config() -> WorkerConfig {
    WorkerConfig {
        // Interval crons shorter than the poll are capped at the poll rate;
        // the worker warns at startup if a schedule is affected.
        cron_poll_interval: Duration::from_secs(5),
        // High-frequency crons write a finished row per occurrence.
        finished_job_retention: Some(Duration::from_secs(3 * 24 * 60 * 60)),
        ..WorkerConfig::default()
    }
}
```

Handlers that must not run until the process itself is ready (compute clients, Kubernetes, template ConfigMaps) can consult `pgspawn::worker_started_at()` and return early. That skip is per claiming worker, which matches the old per-process startup delay.

Cron constructors set `job_key` to the schedule identifier, so a cycle that outlives its interval (or a recovered dead-worker job) does not enqueue a second copy of the same occurrence. Pass `.options(...)` to allow overlap.
...
`pgspawn` logs through the `tracing` crate. Routine cron enqueue, job start, and job success events use `debug`; startup, shutdown, lifecycle summaries, released locks, and recovered jobs use `info`. A failing attempt logs `job failed and will retry` at `warn` with the error and backoff delay, and the final attempt logs `job failed permanently` at `error`. A handler stopped after an operator retires its job logs at `info`, while an unexpected nonterminal lock loss remains an `error`.
...
- `RUST_LOG=info` shows worker lifecycle, summaries, and operational events without routine successful executions.
- `RUST_LOG=pgspawn::worker=debug` adds each cron enqueue, job start, and successful completion.
...
- After an idle claim, each loop reads the earliest runnable `run_at` and sleeps until that deadline or `poll_interval` (30 seconds by default), whichever comes first. The maximum interval covers missed notifications and listener outages without delaying known scheduled work.

Jobs enqueued for immediate execution normally start within milliseconds through notifications. Delayed jobs and retries wake at their known `run_at`; the safety poll bounds delay when a notification is missed or the listener is unavailable. After a handler finishes, its claim loop immediately asks for another job without waiting for either mechanism. Lowering `poll_interval` reduces the missed-notification recovery bound at the cost of more idle database queries.
