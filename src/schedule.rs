...
use crate::{EnqueueOptions, Environment};
...
impl CronJob {
    ...
    pub fn daily(
        ...
        Self {
            ...
            options: EnqueueOptions {
                job_key: Some(identifier),
                ..EnqueueOptions::default()
            },
            catch_up_on_start: false,
            environments: Environment::DEPLOYED.to_vec(),
        }
    }
    ...
    ///
    /// Occurrences are enqueued when a worker's cron poll observes them due, and missed occurrences collapse to the latest, so the effective rate is capped by [`WorkerConfig::cron_poll_interval`](crate::WorkerConfig::cron_poll_interval); lower that setting for sub-poll intervals. The worker logs a warning at startup when a schedule is capped this way. The identifier is also used as the default job key so a still-active occurrence is not stacked.
    pub fn interval(
        identifier: impl Into<String>,
        name: impl Into<String>,
        every: StdDuration,
    ...
    pub fn environments(mut self, environments: impl IntoIterator<Item = Environment>) -> Self {
        ...
        assert!(
            ...
            "cron environments must not be empty"
        );
        self.environments = environments;
        self
    }

    pub fn is_enabled_in(&self, environment: Environment) -> bool {
