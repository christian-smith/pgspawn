...
RETURNING jobs.id,
    ...
    pub(crate) async fn next_runnable_in(
        &self,
        names: &[String],
    ) -> Result<Option<StdDuration>, sqlx::Error> {
        let milliseconds = sqlx::query_scalar::<_, Option<i64>>(
            "
SELECT CASE
    WHEN MIN(candidate_jobs.run_at) IS NULL THEN NULL
    ELSE GREATEST(
        0,
        CEIL(EXTRACT(EPOCH FROM (MIN(candidate_jobs.run_at) - now())) * 1000)::bigint
    )
END
FROM pgspawn.jobs AS candidate_jobs
WHERE candidate_jobs.status = 'queued'
    AND candidate_jobs.name = ANY($1::text[])
    AND (
        candidate_jobs.queue_name IS NULL
        OR NOT EXISTS (
            SELECT 1
        ...
        .fetch_one(&self.pool)
        .await?;
        Ok(milliseconds.map(|value| StdDuration::from_millis(value as u64)))
    }
