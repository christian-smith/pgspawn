...
use crate::{CronJob, CronSchedule, Environment, Job, Queue, Registry, queue::CronEnqueueResult};

const JOB_NOTIFY_CHANNEL: &str = "pgspawn_jobs";
const DATABASE_RETRY_INTERVAL: StdDuration = StdDuration::from_secs(2);

tokio::task_local! {
    static JOB_SHUTDOWN: watch::Receiver<bool>;
    static WORKER_STARTED_AT: Instant;
}
...
/// Instant this worker process began running, for handlers that skip work
/// until the surrounding application has finished starting.
///
/// Returns `None` outside a pgspawn handler. The value is the start of
/// [`Worker::run`] / [`Worker::run_until_shutdown`], not process creation, and
/// is not inherited by tasks created with [`tokio::spawn`].
///
/// ```no_run
/// # use std::time::Duration;
/// # async fn example() {
/// if pgspawn::worker_started_at().is_some_and(|started| started.elapsed() < Duration::from_secs(60)) {
///     return;
/// }
/// # }
/// ```
pub fn worker_started_at() -> Option<Instant> {
    WORKER_STARTED_AT.try_with(|started| *started).ok()
}
...
pub struct WorkerConfig {
    ...
    /// The environment this worker runs in. A worker with cron schedules
    /// refuses to start without one, so a schedule is never silently skipped.
    pub environment: Option<Environment>,
    pub concurrency: usize,
    /// Maximum time idle claim loops wait before checking for missed notifications. A queued job with an earlier `run_at` shortens the wait to its deadline.
    pub poll_interval: StdDuration,
    /// How often the cron loop looks for due schedules. Interval crons
    /// shorter than this are capped at the poll rate; the worker warns at
...
pub enum WorkerError {
    ...
    #[error("cron job `{0}` has no environments")]
    EmptyCronEnvironments(String),
...
impl JobSummary {
    fn average_duration_ms(&self) -> Option<u64> {
        let completed = self.succeeded + self.failed + self.abandoned;
        self.total_elapsed_ms.checked_div(completed)
    }
}
...
pub struct Worker {
    ...
    config: WorkerConfig,
    notify: Arc<Notify>,
    job_summaries: Arc<Mutex<HashMap<String, JobSummary>>>,
    job_summary_interval: Option<StdDuration>,
    started_at: Instant,
}
...
impl Worker {
    pub fn new(pool: PgPool, registry: Registry) -> Self {
        Self {
            ...
            job_summaries: Arc::new(Mutex::new(HashMap::new())),
            job_summary_interval: None,
    ...
    /// Enables periodic aggregate lifecycle logs alongside the default
    /// per-execution debug logs. A zero interval leaves aggregation disabled.
    /// Immediate warnings and errors are never aggregated away.
    pub fn with_job_summary_interval(mut self, interval: StdDuration) -> Self {
        self.job_summary_interval = (!interval.is_zero()).then_some(interval);
        self
    }
    ...
    pub async fn run_until_shutdown<F>(mut self, shutdown: F) -> Result<(), WorkerError>
    where
        F: Future<Output = ()>,
    {
        ...
        for cron in self.crons.iter() {
            ...
            if cron.environments.is_empty() {
                return Err(WorkerError::EmptyCronEnvironments(cron.identifier.clone()));
            }
            // Interval occurrences are enqueued at cron-poll time and missed
            // occurrences collapse to the latest, so an interval shorter than
            // the poll runs at the poll rate instead of its own.
            if let CronSchedule::Interval { every } = &cron.schedule
                && *every < self.config.cron_poll_interval
            {
                warn!(
                    identifier = %cron.identifier,
                    interval_secs = every.as_secs(),
                    cron_poll_interval_secs = self.config.cron_poll_interval.as_secs(),
                    "cron interval is shorter than cron_poll_interval; occurrences are capped at the poll rate -- lower WorkerConfig::cron_poll_interval to honor this schedule"
                );
            }
        }

        // Every schedule is validated above so a misconfigured schedule fails at
        // startup in every environment, not only in the ones that run it.
        if !self.crons.is_empty() {
            let Some(environment) = self.config.environment else {
                return Err(WorkerError::InvalidConfig(
                    "environment must be set when cron schedules are attached",
                ));
            };
            self.crons = Arc::new(
                self.crons
                    .iter()
                    .filter(|cron| cron.is_enabled_in(environment))
                    .cloned()
                    .collect(),
            );
        }
        ...
        info!(
            ...
            environment = self.config.environment.map_or("unset", Environment::as_str),
        ...
        if let Some(interval) = self.job_summary_interval {
            let summary_worker = self.clone();
            let summary_shutdown = shutdown_rx.clone();
            tasks.spawn(async move {
                summary_worker
                    .run_job_summary_loop(interval, summary_shutdown)
                    .await;
            });
        }
    ...
    async fn run_notify_listener_loop(self, mut shutdown: watch::Receiver<bool>) {
        ...
        let mut reconnect_delay = DATABASE_RETRY_INTERVAL;
        ...
        loop {
            ...
            match PgListener::connect_with(&self.queue.pool).await {
                Ok(mut listener) => match listener.listen(JOB_NOTIFY_CHANNEL).await {
                    Ok(()) => {
                        reconnect_delay = DATABASE_RETRY_INTERVAL;
        ...
        for cron in self.crons.iter() {
            ...
            match self.queue.try_enqueue_cron(cron, due_at, next_run_at).await {
                Ok(CronEnqueueResult::Enqueued(job_id)) => {
                    debug!(%job_id, cron = %cron.identifier, job = %cron.name, "enqueued cron job");
                    self.notify.notify_waiters();
                }
    ...
    async fn run_claim_loop(
        ...
        loop {
            ...
            match self.queue.claim_next(&lock_owner, &names).await {
                ...
                Ok(None) => {
                    let wait_duration = match self.queue.next_runnable_in(&names).await {
                        Ok(next_run_in) => {
                            idle_wait_duration(next_run_in, self.config.poll_interval)
                        }
                        Err(err) => {
                            error!(
                                ?err,
                                worker_index, "failed to inspect the next runnable job"
                            );
                ...
                Err(err) => {
                    ...
                    tokio::select! {
                        ...
                        _ = tokio::time::sleep(DATABASE_RETRY_INTERVAL) => {}
                    }
                }
            }
        }
    ...
    async fn run_claimed_job(
        ...
        self.record_job_started(&job.name);
        debug!(
            worker_index,
            job_id = %job.id,
            job = %job.name,
        ...
        let mut handler_task = AbortOnDrop::new(tokio::spawn(
            JOB_SHUTDOWN
                .scope(
                    shutdown.clone(),
                    WORKER_STARTED_AT.scope(started_at, handler_future),
                )
                .instrument(span),
        ));
        let result = loop {
        ...
        match result {
            Ok(Ok(())) => match self.queue.complete_locked(job_id, lock_owner).await {
                Ok(true) => {
                    self.record_job_succeeded(&job_name, elapsed_ms);
                    debug!(job_id = %job_id, job = %job_name, elapsed_ms, "job succeeded")
                }
                Ok(false) => {
                    self.record_job_abandoned(&job_name, elapsed_ms);
            ...
            Ok(Err(err)) => {
                self.record_job_failed(&job_name, elapsed_ms, attempt >= max_attempts);
    ...
    fn record_job_started(&self, job_name: &str) {
        let mut summaries = self
            .job_summaries
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        let summary = summaries.entry(job_name.to_owned()).or_default();
        summary.started += 1;
        summary.running += 1;
    }

    fn record_job_succeeded(&self, job_name: &str, elapsed_ms: u64) {
        let mut summaries = self
            .job_summaries
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        let summary = summaries.entry(job_name.to_owned()).or_default();
        summary.succeeded += 1;
        summary.running = summary.running.saturating_sub(1);
        summary.total_elapsed_ms = summary.total_elapsed_ms.saturating_add(elapsed_ms);
    }

    fn record_job_failed(&self, job_name: &str, elapsed_ms: u64, permanent: bool) {
        let mut summaries = self
            .job_summaries
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    fn record_job_abandoned(&self, job_name: &str, elapsed_ms: u64) {
        let mut summaries = self
            .job_summaries
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    async fn run_job_summary_loop(
        self,
        interval: StdDuration,
        mut shutdown: watch::Receiver<bool>,
    ) {
        let first_report = tokio::time::Instant::now() + interval;
        let mut ticker = tokio::time::interval_at(first_report, interval);
        ...
        loop {
            tokio::select! {
                ...
                _ = ticker.tick() => self.emit_job_summaries(),
            }
        }
    }

    fn emit_job_summaries(&self) {
        let summaries = drain_job_summaries(&self.job_summaries);

        for (job, summary) in summaries {
            let average_duration_ms = summary.average_duration_ms();
            info!(
                target: "pgspawn::job_summary",
                worker_id = %self.config.worker_id,
                job = %job,
                ...
                permanently_failed = summary.permanent_failed,
...
fn idle_wait_duration(next_run_in: Option<StdDuration>, poll_interval: StdDuration) -> StdDuration {
    next_run_in
        .filter(|until_due| !until_due.is_zero())
        .map(|until_due| until_due.min(poll_interval))
        .unwrap_or(poll_interval)
}

fn drain_job_summaries(store: &Mutex<HashMap<String, JobSummary>>) -> HashMap<String, JobSummary> {
    let mut summaries = store
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let mut snapshot = HashMap::with_capacity(summaries.len());
    for (job_name, summary) in summaries.iter_mut() {
        let running = summary.running;
        let current = std::mem::take(summary);
        if current.started > 0 || running > 0 {
            *summary = JobSummary {
                running,
                ..JobSummary::default()
            };
            snapshot.insert(job_name.clone(), JobSummary { running, ..current });
        }
    }
    snapshot
}
...
mod idle_wait_tests {
    ...
    #[test]
    fn waits_for_the_next_deadline_or_the_safety_poll() {
        let safety_poll = StdDuration::from_secs(30);

        assert_eq!(idle_wait_duration(None, safety_poll), safety_poll);
        assert_eq!(
            idle_wait_duration(Some(StdDuration::from_secs(5)), safety_poll),
            StdDuration::from_secs(5)
        );
        assert_eq!(
            idle_wait_duration(Some(StdDuration::from_secs(60)), safety_poll),
            safety_poll
        );
        assert_eq!(
            idle_wait_duration(Some(StdDuration::ZERO), safety_poll),
            safety_poll
        );
    }
}
...
mod started_at_tests {
    ...
    #[tokio::test]
    async fn worker_started_at_is_none_outside_a_handler() {
        assert!(worker_started_at().is_none());
    }

    #[tokio::test]
    async fn worker_started_at_is_visible_inside_the_handler_scope() {
        let started = Instant::now();
        WORKER_STARTED_AT
            .scope(started, async {
                assert_eq!(worker_started_at(), Some(started));
            })
            .await;
    }
}
...
#[cfg(test)]
mod job_summary_tests {
    use super::*;

    #[test]
    fn summary_with_only_running_jobs_has_no_average_duration() {
    ...
    #[test]
    fn summary_averages_completed_job_durations() {
        let summary = JobSummary {
            succeeded: 2,
            failed: 1,
            total_elapsed_ms: 12,
        ...
        assert_eq!(summary.average_duration_ms(), Some(4));
    }

    #[test]
    fn draining_summaries_resets_window_but_preserves_running_jobs() {
        let store = Mutex::new(HashMap::from([(
            "frequent_job".to_owned(),
            JobSummary {
                started: 4,
                succeeded: 3,
                failed: 1,
        ...
        let first = drain_job_summaries(&store);
        let first = first.get("frequent_job").expect("summary exists");
        assert_eq!(first.started, 4);
        assert_eq!(first.succeeded, 3);
        assert_eq!(first.running, 1);

        let second = drain_job_summaries(&store);
        let second = second
            .get("frequent_job")
            .expect("running job remains visible");
