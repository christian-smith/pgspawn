...
#[test]
fn environment_parsing_uses_standard_names() {
...
#[test]
fn cron_environment_policy_defaults_to_deployed_environments() {
    let cron = CronJob::interval("test", "test", StdDuration::from_secs(60));

    assert!(cron.is_enabled_in(Environment::Production));
    assert!(cron.is_enabled_in(Environment::Staging));
    assert!(!cron.is_enabled_in(Environment::Development));
    assert!(!cron.is_enabled_in(Environment::Local));
    assert!(!cron.is_enabled_in(Environment::Test));
}

#[test]
fn cron_environment_policy_restricts_execution() {
    let cron = CronJob::interval("test", "test", StdDuration::from_secs(60))
...
#[test]
fn cron_constructors_default_job_key_to_the_identifier() {
    let interval = CronJob::interval("tick", "tick", StdDuration::from_secs(15));
    assert_eq!(interval.options.job_key.as_deref(), Some("tick"));

    let daily = CronJob::daily_utc("daily-report", "report", DailyJobSchedule::new(1, 0));
    assert_eq!(daily.options.job_key.as_deref(), Some("daily-report"));

    let monthly =
        CronJob::monthly_utc("monthly-report", "report", MonthlyJobSchedule::new(1, 3, 0));
    assert_eq!(monthly.options.job_key.as_deref(), Some("monthly-report"));
}
