...
use pgspawn::{
    CronJob, DailyJobSchedule, EnqueueOptions, EnqueueRequest, Environment, Job, JobKeyMode,
    JobStatus, Queue, Registry, Worker, WorkerConfig, db,
};
...
async fn delayed_job_runs_after_due_time() {
    ...
    let id = queue
        .enqueue_with_options(
            ...
            json!({}),
            EnqueueOptions {
                run_at: Some(jiff::Timestamp::now() + jiff::SignedDuration::from_millis(300)),
                ..EnqueueOptions::default()
            },
        )
        .await
    ...
    let job = wait_for_status(&queue, id, JobStatus::Succeeded, Duration::from_secs(3)).await;
    assert!(
...
async fn interval_cron_coordinates_across_workers() {
    ...
    let registry = Registry::builder()
        ...
        .build();
    let crons = vec![
        CronJob::interval("tick", "tick", Duration::from_secs(1))
...
#[tokio::test]
async fn cron_schedules_require_a_worker_environment() {
    let db = require_db!();
    let registry = Registry::builder()
        .register("tick", |_job| async move { Ok::<(), String>(()) })
        .build();
    let cron = CronJob::interval("tick", "tick", Duration::from_secs(1));
    let config = WorkerConfig {
        environment: None,
        ..fast_config()
    };

    let error = Worker::new(db.pool.clone(), registry)
        .with_config(config)
        .with_crons(vec![cron])
        .run_until_shutdown(std::future::ready(()))
        .await
        .expect_err("a worker with cron schedules must declare its environment");
    assert!(matches!(error, pgspawn::WorkerError::InvalidConfig(_)));

    db.drop().await;
}

#[tokio::test]
async fn cron_schedule_validation_precedes_environment_filtering() {
    let db = require_db!();
    let registry = Registry::builder()
        .register("tick", |_job| async move { Ok::<(), String>(()) })
        .build();
    let missing_handler = CronJob::interval("orphan", "orphan", Duration::from_secs(1))
        .environments([Environment::Production]);
    let error = Worker::new(db.pool.clone(), registry.clone())
        .with_config(fast_config())
        .with_crons(vec![missing_handler])
        .run_until_shutdown(std::future::ready(()))
        .await
        .expect_err("a schedule without a handler fails even where it would not run");
    assert!(matches!(error, pgspawn::WorkerError::MissingCronHandler(name) if name == "orphan"));

    let mut unreachable = CronJob::interval("tick", "tick", Duration::from_secs(1));
    unreachable.environments.clear();
    let error = Worker::new(db.pool.clone(), registry)
        .with_config(fast_config())
        .with_crons(vec![unreachable])
        .run_until_shutdown(std::future::ready(()))
        .await
        .expect_err("a schedule enabled nowhere is a configuration error");
    assert!(matches!(
        error,
        pgspawn::WorkerError::EmptyCronEnvironments(identifier) if identifier == "tick"
    ));
...
#[tokio::test]
async fn cron_environment_policy_prevents_enqueue() {
    let db = require_db!();
    let registry = Registry::builder()
        .register(
            "production-only",
            |_job| async move { Ok::<(), String>(()) },
        )
        .build();
    let cron = CronJob::interval("production-only", "production-only", Duration::from_secs(1))
        .catch_up_on_start()
        .environments([Environment::Production]);
    let config = WorkerConfig {
    ...
    let worker = start_worker_with_crons(db.pool.clone(), registry, config, vec![cron]);

    tokio::time::sleep(Duration::from_millis(300)).await;
    let (jobs,): (i64,) =
        sqlx::query_as("SELECT count(*) FROM pgspawn.jobs WHERE name = 'production-only'")
            .fetch_one(&db.pool)
            .await
            .expect("count production-only jobs");
    let (crons,): (i64,) =
        sqlx::query_as("SELECT count(*) FROM pgspawn.crons WHERE identifier = 'production-only'")
            .fetch_one(&db.pool)
            .await
            .expect("count production-only cron schedules");
